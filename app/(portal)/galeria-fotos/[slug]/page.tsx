...
export default function DetalleGaleriaPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const { data: galeria, isLoading, error } = useSWR(
    slug ? `/api/portal/galerias/${slug}` : null,
    () => galeriasApi.getBySlug(slug)
  );

  if (error) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
      <h2 className="text-2xl font-serif font-bold text-brand-950 mb-4">Álbum no encontrado</h2>
      <Link href="/galeria-fotos" className="text-brand-600 font-bold hover:underline flex items-center gap-2">
        <ArrowLeft className="w-4 h-4" /> Volver a la galería
      </Link>
    </div>
  );

  if (isLoading || !galeria) return <Loader text="Cargando álbum..." />;
