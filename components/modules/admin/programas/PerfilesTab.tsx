...
export function PerfilesTab({ formData, setFormData }: PerfilesTabProps) {
  ...
  const ListBuilder = ({ title, field, dataList }: { title: string, field: string, dataList: string[] }) => (
    <div className="bg-white p-6 rounded-2xl border border-border shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-black text-brand-950">{title}</h3>
        <button
          type="button"
          onClick={() => updateArray(field, [...dataList, ""])}
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <ListBuilder title="Objetivos del Programa" field="objetivos" dataList={objetivos} />
      <ListBuilder title="Perfil del Ingresante (Estudiante)" field="perfil_estudiante" dataList={perfilEstudiante} />
      <ListBuilder title="Perfil del Egresado" field="perfil_egresado" dataList={perfilEgresado} />
    </div>
  );
}
